...
function mixinDiscovery(MsSQL) {
  ...
  MsSQL.prototype.buildQueryForeignKeys = function(owner, table) {
    var sql =
      ...
      ' kcu2.TABLE_SCHEMA AS "pkOwner", rc.UNIQUE_CONSTRAINT_NAME AS "pkName",' +
      ' kcu2.TABLE_NAME AS "pkTableName", kcu2.COLUMN_NAME AS "pkColumnName"' +
      ...
      ' ON kcu2.CONSTRAINT_SCHEMA = rc.UNIQUE_CONSTRAINT_SCHEMA AND kcu2.CONSTRAINT_NAME = rc.UNIQUE_CONSTRAINT_NAME' +
   ...
   * They are ordered by fkOwner, fkTableName, and keySeq.
   * @param owner
   * @param table
   * @returns {string}
   */
  MsSQL.prototype.buildQueryExportedForeignKeys = function(owner, table) {
    var sql =
      ...
      ' kcu.ORDINAL_POSITION IS NOT NULL';
    if (owner) {
      sql += ' AND kcu2.TABLE_SCHEMA=\'' + owner + '\'';
    }
    if (table) {
      sql += ' AND kcu2.TABLE_NAME=\'' + table + '\'';
    }
