...
MsSQL.prototype.getAddModifyColumns = function(model, actualFields) {
  // var sql = [];
  var self = this;
  // sql = sql.concat(self.getColumnsToAdd(model, actualFields));
  // return sql;

  return self.getColumnsToAdd(model, actualFields);
};
...
MsSQL.prototype.getColumnsToAdd = function(model, actualFields) {
    ...
    if (columnsToAdd.length) {
      // sql.push('ALTER TABLE ' + self.tableEscaped(model) + ' ADD ' +
    ...
    if (columnsToAlter.length > 0) {  
      ...
      columnsToAlter.forEach(function(c) {
        // sql.push('ALTER TABLE ' + self.tableEscaped(model) + ' ALTER COLUMN ' +
        //   c);
        sql.push(' ALTER COLUMN ' + c);
      });
    }
  ...
  MsSQL.prototype.getDropColumns = function(model, actualFields) {
    var sql = [];
    var self = this;
  ...
  MsSQL.prototype.getColumnsToDrop = function(model, actualFields) {
    ...
    if (fields) {
      ...
      if (columnsToDrop.length) {
         sql.push(' DROP COLUMN' + columnsToDrop.join(',' + MsSQL.newline));
        // sql.push('ALTER TABLE ' + self.tableEscaped(model) + ' DROP COLUMN' +
  ...
  MsSQL.prototype.removeIndexes = function(model, actualIndexes, cb) {
    var ai = {};
    if (actualIndexes) {
  ...
  MsSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    var self = this;
    ...
    var addSingleColumnIndexes = function(sql, m, propNames, ai) {
      propNames.forEach(function(propName) {
    ...
    var addMultiColumnIndexes = function(sql, m, indexNames, ai) {
    ...
    addSingleColumnIndexes(sql, m, propNames, ai);
    addMultiColumnIndexes(sql, m, indexNames, ai);
  ...
  MsSQL.prototype.alterTable = function(model, actualFields, actualIndexes, done, checkOnly) {
    ...
    statements = statements.concat(self.removeIndexes(model, actualIndexes, function(err, results){}));
    ...
    self.addIndexes(model, actualIndexes, function(err, results){
    });
  };
