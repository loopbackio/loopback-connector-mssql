...
MsSQL.prototype.create = function (model, data, callback) {
  ...
  var isIdentity = (props[modelPKID].type === Number &&
    props[modelPKID].generated !== false);

  if (isIdentity && data[modelPKID] != null) {
  ...
  this.query(sql, fieldsAndData.params, function (err, results) {
    ...
    // executed, we're only interested in the one that returns with the insertId
...
MsSQL.prototype.destroyAll = function (model, where, cb) {
  ...
  var sql = 'DELETE FROM ' + this.tableEscaped(model) + ' ' +
      this.buildWhere(model, where || {});

  // attach a secondary query to get the number of affected items
  var affectedCountQuery = ';SELECT @@ROWCOUNT as count';
  sql += affectedCountQuery;

  this.query(sql, function(err, info) {
    if (err) return cb && cb(err);

    var affectedCountQueryResult = info[0];
    var affectedCount = typeof affectedCountQueryResult.count === 'number' ?
        affectedCountQueryResult.count : undefined;

    cb && cb(null, {count: affectedCount});
  });
};
...
MsSQL.prototype.update = MsSQL.prototype.updateAll = function (model, where, data, cb) {
  ...
  self.query(sql, fieldValues, function(err, info) {
    if (err) return cb && cb(err);
