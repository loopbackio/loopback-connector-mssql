...
MsSQL.prototype.define = function (modelDefinition) {
  ...
  //default pkid is "id"
  var id = "id";
  //override the default with another convention, 'TableName'ID, if defined in the connector settings
  if (this.tableNameID) {
    id = modelDefinition.model.modelName + "ID";
  }
  //override both defaults if a primaryKey is specified in a property
  Object.keys(modelDefinition.properties).forEach(function (propName) {
    var propVal = modelDefinition.properties[propName];
    if (typeof propVal === "object" && propVal.primaryKey) {
      return id = propName;
    }
  });
  this._pkids[modelDefinition.model.modelName] = id;
  ...
  if (id > 0) {
    self.exists(model, id, function (err, yn) {
      ...
      if (yn) {
        ...
        sql += "INSERT INTO " + tblName + " ([" + modelPKID + "]," + fieldNames.join() + ")" + MsSQL.newline;
...
MsSQL.prototype.updateAttributes = function (model, id, data, cb) {
  ...
  this.exists(model, id, function (err, yn) {
    ...
    return cb && cb("A " + tblName + " doesn't exist with a " + modelPKID + " of " + id, id);
  });
};
...
MsSQL.prototype.destroyAll = function (model, where, cb) {
  ...
  this.query('DELETE FROM '
    + this.tableEscaped(model) + ' ' + this.buildWhere(where || {}, props),
    function (err, data) {
    cb && cb(err, data);
  }.bind(this));
...
MsSQL.prototype.tableEscaped = function (model) {
  return '[dbo].[' + this.escapeName(this.table(model)) +']';
};
...
MsSQL.prototype.all = function (model, filter, callback) {
  ...
  if (filter) {
    ...
    if (filter.order) {
      sql += this.buildOrderBy(model, filter.order) + MsSQL.newline;
    }

    if (filter.limit || filter.offset || filter.skip) {
      if (!filter.order) {
        var idName = self.idName(model);
        sql += 'ORDER BY ' + self.columnEscaped(model, idName) + MsSQL.newline;
      }
      sql += buildLimit(filter.limit, filter.offset || filter.skip);
    }
  }
...
MsSQL.prototype.singleIndexSettingsSQL = function (model, prop, add) {
  ...
  var cmd = "CREATE " + (unique ? "UNIQUE " : "") + kind + " INDEX [" + name + "] ON " + tblName + MsSQL.newline;
  cmd += "(" + MsSQL.newline;
  cmd += "    [" + prop + "] " + type;
...
MsSQL.prototype.dropTable = function (model, cb) {
  var tblName = this.tableEscaped(model);
  var cmd = "IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'" + tblName + "') AND type in (N'U'))";
...
MsSQL.prototype.createTable = function (model, cb) {
  ...
  cmd += "IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'" + tblName + "') AND type in (N'U'))" + MsSQL.newline + "BEGIN" + MsSQL.newline;
