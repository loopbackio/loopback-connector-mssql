...
MsSQL.prototype.updateOrCreate = function (model, data, callback) {
  ...
  var sql = "";
  if (id > 0) { //assume that since we've been given an id that they're trying to update this object
...
MsSQL.prototype.updateAttributes = function (model, id, data, cb) {
  ...
  this.exists(model, id, function(err, yn) {
    ...
    if (yn) {
      ...
      return this.updateOrCreate(model, data, cb);
    }
    return callback("A " + tblName + " doesn't exist with a " + modelPKID + " of " + id , id);
  });
};
