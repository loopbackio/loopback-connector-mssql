...
exports.initialize = function initializeSchema(dataSource, callback) {
  if (!mssql || !dataSource.settings.host || !dataSource.settings.database) {
    return;
  }

  var conn_str = "Driver={SQL Server Native Client 11.0};Server=" + dataSource.settings.host + ";";
  var trusted_str = "Trusted_Connection={Yes};Database={" + dataSource.settings.database + "};";

  //if we have a username and password then we use a credential connection string
  if (dataSource.settings.username && dataSource.settings.password) {
    conn_str += "UID=" + dataSource.settings.username + ";PWD="
      + dataSource.settings.password + ";Database={"
  ...
  mssql.open(conn_str, function (err, conn) {
    ...
    dataSource.connector = new MsSQL(dataSource.client);
...
//unchanged from MySql connector, credit to dgsan
