...
MsSQL.prototype.buildPartitionBy = function(model, where) {
  if (!where) {
    return '';
  }
  ...
  for (var key in where) {
    ...
    var columnName = this.columnEscaped(model, key);
    break;
  }

  return 'PARTITION BY ' + columnName;
};
...
MsSQL.prototype.buildColumnNames = function(model, filter) {
  ...
  if (filter.limit || filter.offset || filter.skip) {
    ...
    if(filter.where) {
      partitionByClause = this.buildPartitionBy(model, filter.where);
    }
    if (orderBy) {
