...
function MsSQL(settings) {
  this.constructor.super_.call(this, name, settings);
  // this.name = name;
...
MsSQL.prototype.tableEscaped = function (model) {
  ...
  var schemaName = (mssqlMeta && (mssqlMeta.schema || mssqlMeta.schemaName)) || 'dbo';
  return '[' + schemaName + '].[' + this.escapeName(this.table(model)) + ']';
};
