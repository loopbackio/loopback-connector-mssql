  ...
  if (filter) {
    ...
    if (filter.limit || filter.offset || filter.skip) {
      if (!filter.order) {
        ...
        orderBy = 'ORDER BY ' + self.columnEscaped(model, idName)
        sql += orderBy + MsSQL.newline;
      }
      ...
      if (this.settings.supportsOffsetFetch) {
        ...
        var paginatedSQL = 'SELECT ' + this.getColumns(model, filter.fields) + MsSQL.newline
          ...
          + ' FROM ' + this.tableEscaped(model) + MsSQL.newline;
        if (filter.where) {
          paginatedSQL += this.buildWhere(filter.where, props) + MsSQL.newline;
        }
...
MsSQL.prototype._buildWhere = function (conds, props) {
  // debugger;
  var self = this;
  var cs = [];
  Object.keys(conds).forEach(function (key) {
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        clauses = clauses.map(function (c) {
          return '(' + self._buildWhere(c, props) + ')';
        });
    ...
    if (conds[key] === null) {
      ...
      if (condType === "max") {
        ...
        if (subClause) {
          sqlCond += " " + self._buildWhere(subClause, props);
        }
        sqlCond += ")";
        cs.push(sqlCond);
        return true;
...
MsSQL.prototype.buildWhere = function (conds, props) {
  var where = this._buildWhere(conds, props);
